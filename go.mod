module github.com/dimus/gnidump

require (
	github.com/AndreasBriese/bbloom v0.0.0-20170702084017-28f7e881ca57 // indirect
	github.com/davecgh/go-spew v1.1.1 // indirect
	github.com/dgraph-io/badger v0.0.0-20170905012857-f407625a0c13
	github.com/go-sql-driver/mysql v0.0.0-20170822214809-26471af196a1
	github.com/golang/protobuf v0.0.0-20170902000452-17ce1425424a // indirect
	github.com/google/gofuzz v0.0.0-20170612174753-24818f796faf // indirect
	github.com/json-iterator/go v0.0.0-20170905050003-db32ee8c2dc8
	github.com/pkg/errors v0.0.0-20170505043639-c605e284fe17 // indirect
	github.com/pmezard/go-difflib v1.0.0 // indirect
	github.com/satori/go.uuid v0.0.0-20170321230731-5bf94b69c6b6
	github.com/stretchr/testify v1.2.2 // indirect
	golang.org/x/net v0.0.0-20170828231752-66aacef3dd8a // indirect
	golang.org/x/sync v0.0.0-20181108010431-42b317875d0f // indirect
	golang.org/x/sys v0.0.0-20170901181214-7ddbeae9ae08 // indirect
)
